...
The workshop tracked orders on paper: dimensions, materials, and cutting specifications written by hand. Slips were lost, illegible, or duplicated, and pricing was recalculated from memory each time.

## The Solution

A Spring Boot web app that digitizes the entire order lifecycle — from creating an order with multiple cut lines, through automatic price calculation, to status tracking until pickup. Every screen is optimized for the single-tablet workflow and usability over a slow network.

## Public Landing Page

The project includes a simple public-facing landing page to provide:
- Quick access to **demo credentials** (`demo` / `demo123`).
- A high-level overview of the app's purpose and functionalities.

Available at: [oction.up.railway.app](https://oction.up.railway.app).

## Key Engineering Decisions

| Decision | Why |
|----------|-----|
| **Server-rendered Thymeleaf + Alpine.js** | The shop runs on a single tablet — no SPA build step, no auth-token shuffling, fast first paint over 4G. Alpine handles the small interactive bits (e.g. forms, dashboards). |
...
- **LinieComanda** (Order line) — material reference, dimensions (length × width × thickness), `TIP_CANT` (edge finish style), four boolean flags for which sides are finished, four flags for which corners are finished.  
